    ...
    <div class="card-body">
      <form @submit="submitForm">
        ...
        <div class="form-group">
          ...
          <input type="date" id="fromDate" v-model="leave.fromDate" class="form-control " :min="minDate" required>
        </div>

        <div class="form-group">
          <label for="toDate">To Date:</label>
          <input type="date" id="toDate" v-model="leave.toDate" class="form-control" :min="minDate" required>
        </div>
...
export default {
  ...
  methods: {
    submitForm() {
      axios.post(`https://hope1.onrender.com/requestLeave/${this.leave.id}`, this.leave)
        .then(response => {
          console.warn(response);
          console.log('Leave request has been sent');
          this.showToast = true;
          this.toastTitle = 'Leave was approved';
          setTimeout(() => {
