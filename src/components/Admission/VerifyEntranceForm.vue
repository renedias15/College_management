    ...
    <table class="table table-striped table-bordered">
      ...
      <tbody>
        <tr v-for="ent_form in filteredForms" :key="ent_form[0]">
          ...
          <td>{{ ent_form[2] }}</td>
          <td>{{ ent_form[3] }}</td>
          <td> <a :href="ent_form[4]" target="_blank">Image</a></td>
          <td> <a :href="ent_form[5]" target="_blank">Image</a></td>
          <td>
            <button class="btn btn-success mr-2" @click="Elligible(ent_form[0],'A')">Accept</button>
            <button class="btn btn-danger" @click="NonElligible(ent_form[0],'R')">Reject</button>
          </td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  methods: {
    fetchForms() {
      axios.get('https://hope1.onrender.com/getEnteranceForms')
        .then(response => {
    ...
    Elligible(id,res){
        ...
        axios.put(`https://hope1.onrender.com/verifyApplicant/${id}/${res}`)
        .then(response => {
